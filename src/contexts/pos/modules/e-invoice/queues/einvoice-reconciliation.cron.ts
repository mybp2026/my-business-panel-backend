...
@Injectable()
export class EInvoiceReconciliationCron {
  private readonly logger = new Logger(EInvoiceReconciliationCron.name);
  ...
  async reconcile(): Promise<void> {
    ...
    for (const invoice of pendingInvoices) {
      if (activeInvoiceIds.has(invoice.electronic_sale_invoice_id)) continue;
    ...
    if (requeued > 0) {
      this.logger.warn(`Reconciliation: re-enqueued ${requeued} orphaned invoices`);
    }
  }
}
